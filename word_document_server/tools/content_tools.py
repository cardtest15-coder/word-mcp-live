        ...
        async with get_file_lock(filename):
            ...
            for paragraph in doc.paragraphs:
                ...
                try:
                    ...
                    logging.getLogger(__name__).warning("Could not copy paragraph style: %s", e)
...
async def insert_header_near_text_tool(filename: str, target_text: Optional[str] = None, header_title: str = "", position: str = 'after', header_style: str = 'Heading 1', target_paragraph_index: Optional[int] = None) -> str:
...
async def insert_numbered_list_near_text_tool(filename: str, target_text: Optional[str] = None, list_items: Optional[list] = None, position: str = 'after', target_paragraph_index: Optional[int] = None, bullet_type: str = 'bullet') -> str:
...
async def insert_line_or_paragraph_near_text_tool(filename: str, target_text: Optional[str] = None, line_text: str = "", position: str = 'after', line_style: Optional[str] = None, target_paragraph_index: Optional[int] = None) -> str:
...
async def replace_paragraph_block_below_header_tool(filename: str, header_text: str, new_paragraphs: list) -> str:
    """Replace the paragraph block below a header, avoiding modifying the TOC."""
    async with get_file_lock(filename):
        return replace_paragraph_block_below_header(filename, header_text, new_paragraphs)

async def replace_block_between_manual_anchors_tool(filename: str, start_anchor_text: str, new_paragraphs: list, end_anchor_text: Optional[str] = None, new_paragraph_style: Optional[str] = None) -> str:
    """Replace all content between start_anchor_text and end_anchor_text (or next logical header if not provided)."""
    async with get_file_lock(filename):
        return replace_block_between_manual_anchors(filename, start_anchor_text, new_paragraphs, end_anchor_text, None, new_paragraph_style)
